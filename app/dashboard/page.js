...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/favorites")
      .then((r) => r.json())
      .then((data) => { setFavorites(Array.isArray(data) ? data : []); setLoading((p) => ({ ...p, favorites: false })); })
  ...
  useEffect(() => {
    if (!session?.user?.id) return;
    Promise.all([
      fetch("/api/products/mine").then((r) => r.json()).catch(() => []),
      fetch("/api/chat/conversations").then((r) => r.json()).catch(() => []),
    ]).then(([products, conversations]) => {
      const events = [];
      if (Array.isArray(products)) {
        products.forEach((p) => {
          events.push({ type: "listing", product: p, date: new Date(p.createdAt).getTime(), label: "createdListing" });
        });
      }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {tab === "favorites" && (
        <FavoritesTab favorites={favorites} loading={loading.favorites} t={t} favIds={favIds} />
      )}
      {tab === "listings" && (
...
function FavoritesTab({ favorites, loading, t, favIds }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
      {favorites.map((p) => <ProductCard key={p.id} product={p} favoritedIds={favIds} />)}
    </div>
  );
}
  ...
  if (listings.length === 0) {
    return (
      <div className="bg-white border border-slate-100 rounded-2xl p-12 text-center">
        ...
        <Link href="/products/new" className="mt-4 inline-block bg-indigo-600 text-white px-5 py-2 rounded-full font-medium hover:bg-indigo-700 transition">
          {t("postFirstListing")}
        </Link>
      </div>
    );
...
function ActivityRow({ event, t }) {
  ...
  const time = date.toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });

  if (event.type === "listing") {
    return (
      <Link href={"/products/" + event.product.id}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm text-slate-700 truncate">{t("createdListing")}: <span className="font-medium">{event.product.title}</span></p>
  ...
  return (
    <Link href={"/chat/" + event.conversation.id}
      className="flex items-center gap-3 bg-white rounded-xl border border-slate-100 p-3 hover:shadow-sm transition"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-slate-700 truncate">{t("messagedAbout")}: <span className="font-medium">{event.productTitle || event.conversation.product?.title}</span></p>
        <p className="text-xs text-slate-400">{time}</p>
      </div>
    </Link>
  );
