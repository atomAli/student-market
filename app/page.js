    ...
    fetch("/api/products?" + params.toString())
      ...
      .then((data) => {
        if (!cancelled && Array.isArray(data)) {
          const filtered = data.filter((p) => !p.sold);
    ...
    <div>
      ...
      <div className="flex gap-2 flex-wrap mb-8 justify-center">
        <button
          ...
          className={
            "flex items-center gap-1 px-4 py-1.5 rounded-full text-sm font-medium transition border " +
            (category === "all"
              ? "text-white"
      ...
      <div className="text-center mt-12 py-6 border-t border-slate-100">
        <a
          href="https://revolut.me/arjomandi"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-slate-400 hover:text-indigo-500 transition text-sm"
        >
