    ...
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="bg-white p-8 rounded-2xl shadow-md border border-slate-100 w-full max-w-md">
        <div className="text-center mb-6">
          <FileText className="w-10 h-10 mx-auto mb-2" style={{ color: "var(--color-primary)" }} />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              required className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">{t("passwordLabel")}</label>
            <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}
              required minLength={6} placeholder={t("minLength")}
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
          </div>
        ...
        <p className="text-center text-sm text-slate-400 mt-5">
          {t("hasAccount")}{" "}
          <Link href="/login" className="text-indigo-600 hover:underline font-semibold">{t("loginLink")}</Link>
        </p>
      </div>
    </div>
  );
