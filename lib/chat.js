...
export async function getUnreadCount(userId) {
  ...
  const rows = await prisma.$queryRawUnsafe(
    ...
    WHERE m."senderId" != $1
      ...
      AND (c."buyerId" = $2 OR p."sellerId" = $3)
  `,
    userId,
    userId,
    userId,
  ...
  const rows = session.user.isAdmin
    ...
    : await prisma.$queryRawUnsafe(
        ...
        SELECT
          ...
          lsender.name AS "lastMessageSenderName",
          lsender.email AS "lastMessageSenderEmail",
          (SELECT COUNT(*) FROM "Message" m2 WHERE m2."conversationId" = c.id AND m2."senderId" != $1 AND m2."readAt" IS NULL) AS "unreadCount"
        FROM "Conversation" c
        JOIN "Product" p ON p.id = c."productId"
        JOIN "User" buyer ON buyer.id = c."buyerId"
        JOIN "User" seller ON seller.id = p."sellerId"
        ...
        LEFT JOIN "User" lsender ON lsender.id = lm."senderId"
        WHERE c."buyerId" = $2 OR p."sellerId" = $3
        ORDER BY c."updatedAt" DESC
      `,
        session.user.id,
        session.user.id,
